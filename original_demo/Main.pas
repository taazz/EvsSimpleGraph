...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure SimpleGraphNodeDblClick(Graph: TEvsSimpleGraph;
      Node: TEvsGraphNode);
    procedure SimpleGraphLinkDblClick(Graph: TEvsSimpleGraph;
      Link: TEvsGraphLink);
    procedure FormCreate(Sender: TObject);
    procedure SimpleGraphCommandModeChange(Sender: TObject);
    procedure SimpleGraphObjectSelect(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject);
    procedure SimpleGraphObjectDblClick(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject);
    ...
    procedure SimpleGraphInfoTip(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject; var InfoTip: String);
    ...
    procedure SimpleGraphCanHookLink(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject; Link: TEvsGraphLink; Index: Integer;
      var CanHook: Boolean);
    procedure SimpleGraphCanLinkObjects(Graph: TEvsSimpleGraph;
      Link: TEvsGraphLink; Source, Target: TEvsGraphObject;
      var CanLink: Boolean);
    ...
    procedure SimpleGraphCanRemoveObject(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject; var CanRemove: Boolean);
    procedure ViewWholeGraphExecute(Sender: TObject);
    procedure SimpleGraphObjectChange(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject);
    procedure ExportBitmapExecute(Sender: TObject);
    procedure SimpleGraphObjectInitInstance(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject);
    ...
    procedure SimpleGraphObjectAfterDraw(Graph: TEvsSimpleGraph;
      GraphObject: TEvsGraphObject; Canvas: TCanvas);
    procedure ViewPanExecute(Sender: TObject);
    procedure SimpleGraphNodeMoveResize(Graph: TEvsSimpleGraph;
      Node: TEvsGraphNode);
  ...
  private
    ...
    function ForEachCallback(GraphObject: TEvsGraphObject; Action: Integer): Boolean;
  public
...
function TMainForm.ForEachCallback(GraphObject: TEvsGraphObject; Action: Integer): Boolean;
var
  RotateOrg: TPoint;
begin
  Result := True;
...
begin
  ...
  SimpleGraph:= TEvsSimpleGraph.Create(Self);
    SimpleGraph.Left                   := 0;
    SimpleGraph.Top                    := 54;
    ...
    SimpleGraph.Font.Color             := clNavy;
    SimpleGraph.Font.Height            := -13;
    SimpleGraph.Font.Name              := 'Arial';
    SimpleGraph.Font.Style             := [fsBold];
    SimpleGraph.HorzScrollBar.Margin   := 8;
    SimpleGraph.HorzScrollBar.Smooth   := True;
    SimpleGraph.HorzScrollBar.Tracking := True;
    SimpleGraph.ObjectPopupMenu        := ObjectsPopup;
    SimpleGraph.ParentColor            := False;
    SimpleGraph.ParentFont             := False;
    SimpleGraph.ParentShowHint         := False;
    SimpleGraph.PopupMenu              := DesignerPopup;
    SimpleGraph.ShowHint               := True;
    SimpleGraph.TabOrder               := 0;
    SimpleGraph.VertScrollBar.Margin   := 8;
    SimpleGraph.VertScrollBar.Smooth   := True;
    SimpleGraph.VertScrollBar.Tracking := True;
    SimpleGraph.OnDblClick             := SimpleGraphDblClick;
    SimpleGraph.OnMouseMove            := SimpleGraphMouseMove;
    SimpleGraph.OnMouseWheelDown       := SimpleGraphMouseWheelDown;
    SimpleGraph.OnMouseWheelUp         := SimpleGraphMouseWheelUp;
    SimpleGraph.OnObjectInitInstance   := SimpleGraphObjectInitInstance;
    SimpleGraph.OnObjectChange         := SimpleGraphObjectChange;
    SimpleGraph.OnObjectSelect         := SimpleGraphObjectSelect;
    SimpleGraph.OnObjectDblClick       := SimpleGraphObjectDblClick;
    SimpleGraph.OnObjectAfterDraw      := SimpleGraphObjectAfterDraw;
    SimpleGraph.OnCanHookLink          := SimpleGraphCanHookLink;
    SimpleGraph.OnCanLinkObjects       := SimpleGraphCanLinkObjects;
    SimpleGraph.OnCanRemoveObject      := SimpleGraphCanRemoveObject;
    SimpleGraph.OnNodeMoveResize       := SimpleGraphNodeMoveResize;
...
begin
  ...
  begin
    ...
    SaveDialog.FileName := ExpandFileNameUTF8(ParamStr(1)); { *Converted from ExpandFileName*  }
...
begin
  ...
  begin
    LinkCount := SimpleGraph.SelectedObjectsCount(TEvsGraphLink);
    if LinkCount = 0 then
...
begin
  ObjectsRectangle.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsRectangularNode);
...
begin
  ObjectsRoundRect.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsRoundRectangularNode);
...
begin
  ObjectsEllipse.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsEllipticNode);
...
begin
  ObjectsTriangle.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsTriangularNode);
...
begin
  ObjectsRhomboid.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsRhomboidalNode);
...
begin
  ObjectsPentagon.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsPentagonalNode);
...
begin
  ObjectsHexagon.Checked :=(SimpleGraph.CommandMode = cmInsertNode) and
    (SimpleGraph.DefaultNodeClass = TEvsHexagonalNode);
...
begin
  with TEvsGraphLink(SimpleGraph.SelectedObjects[0]) do
...
begin
  if (SimpleGraph.SelectedObjects.Count = 1) and
     (SimpleGraph.SelectedObjects[0] is TEvsGraphLink) then
  begin
    ...
    Link := TEvsGraphLink(SimpleGraph.SelectedObjects[0]);
...
procedure TMainForm.SimpleGraphNodeDblClick(Graph: TEvsSimpleGraph;
  Node: TEvsGraphNode);
begin
  EditProperties.Execute;
end;

procedure TMainForm.SimpleGraphLinkDblClick(Graph: TEvsSimpleGraph;
  Link: TEvsGraphLink);
begin
  EditProperties.Execute;
end;

procedure TMainForm.SimpleGraphObjectDblClick(Graph: TEvsSimpleGraph;
  GraphObject: TEvsGraphObject);
begin
  EditProperties.Execute;
end;
...
procedure TMainForm.SimpleGraphNodeMoveResize(Graph: TEvsSimpleGraph;
  Node: TEvsGraphNode);
begin
...
procedure TMainForm.SimpleGraphObjectSelect(Graph: TEvsSimpleGraph;
  GraphObject: TEvsGraphObject);
begin
...
procedure TMainForm.SimpleGraphInfoTip(Graph: TEvsSimpleGraph;
  GraphObject: TEvsGraphObject; var InfoTip: String);
const
...
begin
  if GraphObject.IsLink then
    ...
    with TEvsGraphNode(GraphObject) do
    begin
      InfoTip := Format(SNodeInfo, [PrettyNodeClassName(ClassName),
        Text, Left, Top, Width, Height,
        TEvsGraphNode(GraphObject).LinkInputCount,
        TEvsGraphNode(GraphObject).LinkOutputCount]);
    end;
end;

procedure TMainForm.SimpleGraphCanHookLink(Graph: TEvsSimpleGraph;
  GraphObject: TEvsGraphObject; Link: TEvsGraphLink; Index: Integer;
  var CanHook: Boolean);
const
...
procedure TMainForm.SimpleGraphCanLinkObjects(Graph: TEvsSimpleGraph;
  Link: TEvsGraphLink; Source, Target: TEvsGraphObject; var CanLink: Boolean);
var
  MsgText: String;
...
procedure TMainForm.SimpleGraphCanRemoveObject(Graph: TEvsSimpleGraph;
  GraphObject: TEvsGraphObject; var CanRemove: Boolean);
var
  MsgText: String;
  ObjectName: String;
...
procedure TMainForm.SimpleGraphObjectChange(Graph: TEvsSimpleGraph; GraphObject: TEvsGraphObject);
var
...
procedure TMainForm.SimpleGraphObjectInitInstance(Graph: TEvsSimpleGraph;
  GraphObject: TEvsGraphObject);
var
